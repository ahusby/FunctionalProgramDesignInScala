...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val (b, ms) = initial.head
      val more = for {
        next <- newNeighborsOnly(neighborsWithHistory(b, ms), explored)
      } yield next

      initial.head #:: (from(initial.tail #::: more, explored + b))
    }
  }
  ...
  /**
    ...
    */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter { p =>
    val isOnGoal = done(p._1)
    println(p._1 + " done? " + isOnGoal)
    isOnGoal
  }
